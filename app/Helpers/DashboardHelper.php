...
class DashboardHelper
{
    ...
    public function get_count_notifications($vehicle_id, $service)
    {
        $vehicle = Vehicle::where('odoo_id', $vehicle_id)->first();

        $count_notifications = DB::table('user_notifications')
            ->where('vehicle_id', $vehicle->id)
            ->where('topic_1', $service)
            ->where('status', 'sent')
            ->count();
     ...
     *
     */
    public function get_service_chart($vehicle_id)
    {
        $vehicle = Vehicle::where('odoo_id', $vehicle_id)->first();
        ...
        if ($tire_histories !== null && $tire_histories->count() > 0) {
            ...
            if ($tire_histories->last()->odometer != $tire_histories->first()->odometer) {
                $kms_traveled = $last_history->odometer - $first_history->odometer;
                $proyected_kms = $tire_histories->last()->tread_depth * $kms_traveled;

                $days_elapsed = Carbon::parse($tire_histories->first()->service_date)->diffInDays($tire_histories->last()->service_date);
                $projected_days = round(($days_elapsed * $proyected_kms) / ($kms_traveled));

                $kms_next = round(($tire_histories->last()->odometer + $proyected_kms));
        ...
        $data_balancing = $last_balancing ? [
            ...
            'balancing_notifications' => $this->get_count_notifications($vehicle_id, 'balancing'),
            'no_data_kms' => false,
        ...
        $alignment_data = $last_alignment ? [
            ...
            'percentage_date_good_color' => '#46CA31',
            'percentage_date_bad_color' => '#CC0D0D',
            'alignment_notifications' => $this->get_count_notifications($vehicle_id, 'alignment'),
            'no_data_kms' => false,
        ...
        $data_battery = $last_battery ? [
            ...
            'battery_notifications' => $this->get_count_notifications($vehicle_id, 'battery'),
            'no_data' => false,
        ] : [
            'battery_label' => 'Batería',
            'no_data' => true,
        ...
        $tire_data = [
            ...
            'tire_notifications' => $this->get_count_notifications($vehicle_id, 'tire'),
            'no_data_current' => count($tire_histories) < 1,
            'no_data_next' => count($tire_histories) < 3,
        ];

        if (count($tire_histories) >= 3) {
            ...
            $tire_data['show_change_alert_message'] = $max_lifespan_consumed >= $min_alert && $max_lifespan_consumed < $max_alert;
            $tire_data['show_danger_message'] = $max_lifespan_consumed >= $danger;
        ...
        return [
            'Dashboard' => [
                ...
                'last_store_id' => $service->store_id,
    ...
    private function getOilStats($vehicle_id)
    {
        ...
        if ($last_service_oil) {
            ...
            $response_oil['date_next'] = optional(Carbon::create($last_service_oil->date))->addMonths(6)->format('Y-m-d');
            ...
            $response_oil['oil_notifications'] = $this->get_count_notifications($vehicle_id, 'oil');
    ...
    public function getRotationStats($vehicle_id, $last_service)
    {
        ...
        if ($last_rotation_service) {
            ...
            return [
                ...
                'percentage_date_good_color' => '#46CA31',
                'percentage_date_bad_color' => '#CC0D0D',
                'rotation_notifications' => $this->get_count_notifications($vehicle_id, 'rotation'),
    ...
    private function get_tire_details($vehicle_id)
    {
        ...
        $tire_chart = array_values(array_filter($this->getVehicleTireChart($vehicle_id), function ($item) use ($last_service) {
            return ($item["service_date"] == $last_service->date);
        }));
    ...
    private function get_performance_index($vehicle_id, $tire_location)
    {
        ...
        $vehicle_tire_histories = VehicleTireHistory::where('vehicle_id', $vehicle_id)
            ->where('tire_location', '<>', 'Repuesto')
            ->where('tire_location', $tire_location)->get();

        $response = [];
        if (count($vehicle_tire_histories) <= $min_service_tires_histories) {
            return $response;
        }

        for ($i = 0; $i < count($vehicle_tire_histories); $i++) {
            $response[] = [
                'index' => $i + 1,
                'visit_date' => $vehicle_tire_histories[$i]['service_date'],
                'performance_index' => $vehicle_tire_histories[$i]['performance_index'],
                'km_proyected' => $vehicle_tire_histories[$i]['km_proyected']
            ];
        }
