...
class DashboardController extends Controller
{
    ...
    public function getServiceBattery(DashboardBatteryRequest $request, $vehicle_id)
    {
        ...
        $services = Service::where('vehicle_id', $vehicle->odoo_id)
            ->where('state', 'done')
            ->orderBy('date', 'desc')
            ->get();
    ...
    {
        try {
            $vehicle = Vehicle::where('odoo_id', $vehicle_id)->firstOrFail();

            $latestService = $vehicle->services()
                ->where('state', 'done')
                ->orderByDesc('date')
                ->firstOrFail();

            $serviceBalancing = $latestService->serviceBalancing()
                ->where('location', '!=', 'Repuesto')
                ->get();

            $lastBalancedServiceId = Service::where('vehicle_id', $vehicle_id)
                ->join('service_balancing', 'services.odoo_id', '=', 'service_balancing.service_id')
                ->where('service_balancing.balanced', true)
                ->orderByDesc('services.date')
                ->value('services.odoo_id');

            if ($lastBalancedServiceId) {
                $lastBalanced = ServiceBalancing::where('service_id', $lastBalancedServiceId)->get();

                foreach ($serviceBalancing as $index => $value) {
                    if ($value->location === 'Delantero Izquierdo') {
                        $serviceBalancing[$index]->lead_used = $lastBalanced[0]->lead_used;
                    } elseif ($value->location === 'Delantero Derecho') {
                        $serviceBalancing[$index]->lead_used = $lastBalanced[1]->lead_used;
                    } elseif ($value->location === 'Trasero Izquierdo') {
                        $serviceBalancing[$index]->lead_used = $lastBalanced[2]->lead_used;
                    } elseif ($value->location === 'Trasero Derecho') {
                        $serviceBalancing[$index]->lead_used = $lastBalanced[3]->lead_used;
                    }
                }
            }
    ...
    {
        return response()->json($this->helper->get_service_chart($vehicle_id));
    }
