...
class SyncOdooController extends Controller
{
    ...
    public function __construct(OdooHelper $odoo)
    {
        $this->odoo = $odoo;
    }
    ...
    public function sync(SyncRequest $request): \Illuminate\Http\JsonResponse
    {
        ...
        $response = $this->getServicesFromOdoo($procesado_iron, $state, $page, $size);

        if ($this->shouldSynchronize($response, $page)) {
            $this->synchronize($response->data, $not_processed, $procesado_iron);
    ...
    protected function getServicesFromOdoo(bool $procesado_iron, string $state, int $page, int $size)
    {
        $params = [
        ...
        return $this->odoo->getServices($params);
    }
    ...
    protected function buildFilterParams(bool $procesado_iron, string $state): array
    {
        return [
            [
                'field'    => 'date',
                'operator' => '>=',
                'value'    => Carbon::createFromFormat('Y-m-d', '2024-11-04')
            ],
            [
    ...
    protected function synchronize($data, bool $not_processed, bool $procesado_iron): void
    {
        Bus::batch([
            
            new SyncInProgress($this->modules, $data, $not_processed, $procesado_iron)
