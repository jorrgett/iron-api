...
class OdooSync
{
    ...
    public function run(string $service, array $response, bool $not_processed, bool $procesado_iron): void
    {
        $this->serviceFromOdoo = new OdooHelper();
        ...
        foreach ($response as $data) {
            ...
            $this->saveServiceSynced($data, $procesado_iron);
        }
    ...
    private function saveServiceSynced($data, bool $procesado_iron): void
    {
    ...
    public function processedItems(): void
    {
        ...
        $items = $items = $records->map(function ($item) {
            Log::info($item['service_id']);
            return [
                'id' => $item['service_id'],
                'procesado_iron' => true,
            ];
        ...
        if (!empty($records)){
            dispatch(new SyncProcessed($items));
        }        
    }
