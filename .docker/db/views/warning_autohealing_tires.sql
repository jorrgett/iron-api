CREATE OR REPLACE VIEW public.warning_autohealing_tires
AS SELECT u.res_partner_id,
    ...
    s_tires.tire_change,
        CASE
            WHEN tire_histories.lifespan_consumed <= 0.75::double precision THEN 'Buen Estado'::text
            WHEN tire_histories.lifespan_consumed > 0.75::double precision AND tire_histories.lifespan_consumed <= 0.85::double precision THEN 'Requiere cambio'::text
            WHEN tire_histories.lifespan_consumed > 0.85::double precision THEN 'Peligro'::text
            ELSE NULL::text
        END AS change_status,
        CASE
            WHEN tire_histories.km_proyected > 0 THEN tire_histories.km_traveled::double precision / tire_histories.km_proyected::double precision
            ELSE 0::double precision
        END AS traveled_percentage,
        CASE
        ...
        END AS count_100
   FROM services
     JOIN vehicle_tire_histories tire_histories ON services.odoo_id = tire_histories.service_id
  ...
  WHERE tire_histories.tire_location::text <> 'Repuesto'::text AND tire_histories.mm_consumed < 0::double precision
  ORDER BY v.odoo_id, services.odoo_id, tire_histories.tire_location;